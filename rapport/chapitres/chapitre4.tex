...
% Dans cette section il faut parler de pourquoi la première implémentation qu'on a est très lente (plusieurs dizaines de secondes pour une image) par rapport à la fft de numpy par exemple. On évoque le fait que dans numpy on fait appel à du code C compilé qui est donc beaucoup plus rapide que python et surtout que l'implémentation évidente de l'algorithme n'est pas efficace. On peut écrire une version itérative plutôt que récursive pour gagner en efficacité, on peut également paralléliser les calculs. Eventuellement on peut écrire une version aussi efficace que possible en C dans un autre fichier pour montrer la différence.
